    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          Container(
            ...
            child: IconButton(
              icon: Icon(Icons.tune_outlined, color: AppColors.primary, size: 20),
              onPressed: () {},
            ),
          ),
        ],
      ...
      body: async.when(
        ...
        data: (requests) {
          ...
          final pending = requests.where((r) => _matchesStatus(r, 'pending')).length;
          final approved = requests.where((r) => _matchesStatus(r, 'approved')).length;
          final rejected = requests.where((r) => _matchesStatus(r, 'rejected')).length;
...
class _LeaveRequestCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final leaveType = (request.leaveType ?? request.reference ?? 'Leave Request').toString();
